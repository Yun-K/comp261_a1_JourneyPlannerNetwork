...
/**
 * This is a template GUI that you can use for your mapping program. It is an *abstract
 * class*, which means you'll need to extend it in your own program. For a simple example
 * of how to do this, have a look at the SquaresExample class.
 * 
 * This GUI uses Swing, not the first-year UI library. Swing is not the focus of this
 * course, but it would be to your benefit if you took some time to understand how this
 * class works.
 * 
 * @author tony
 */
public abstract class GUI {
    /**
     * defines the different types of movement the user can perform, the appropriate one
     * is passed to your code when the move(Move) method is called.
     */
    public enum Move {
    ...
    /**
     * Is called when the mouse is clicked (actually, when the mouse is released), and is
     * passed the MouseEvent object for that click.
     */
    ...
    /**
     * Is called whenever a navigation button is pressed. An instance of the Move enum is
     * passed, representing the button clicked by the user.
     */
    protected abstract void onMove(Move m);

    /**
     * Is called when the user has successfully selected a directory to load the data
     * files from.
     * 
     * @param stopFile
     *            the stops.txt file
    ...
    /**
     * @return the JTextField used as a search box in the top-right, which can be queried
     *         for the string it contains.
     */
    ...
    /**
     * Redraws the window (including drawing pane). This is already done whenever a button
     * is pressed or the search box is updated, so you probably won't need to call this.
     */
    public void redraw() {
        frame.repaint();
    }
    ...
    /*
     * In Swing, everything is a component; buttons, graphics panes, tool tips, and the
     * window frame are all components. This is implemented by JComponent, which sits at
     * the top of the component inheritance hierarchy. A JFrame is a component that
     * represents the outer window frame (with the minimise, maximise, and close buttons)
     * of your program. Every swing program has to have one somewhere. JFrames can, of
     * course, have other components inside them. JPanels are your bog-standard container
     * component (can have other components inside them), that are used for laying out
     * your UI.
     */
    ...
    private void initialise() {
        ...
        /*
         * then make the drawing canvas, which is really just a boring old JComponent with
         * the paintComponent method overridden to paint whatever we like. this is the
         * easiest way to do drawing.
         */
        ...
        /*
         * then make the JTextArea that goes down the bottom. we put this in a JScrollPane
         * to get scroll bars when necessary.
         */
         ...
         * complicated than it could be, as we put the drawing and text output components
         * inside a JSplitPane so they can be resized by the user. the JScrollPane and the
         * top bar are then added to the frame.
         */
